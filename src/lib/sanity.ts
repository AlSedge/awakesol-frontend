...
export async function fetchBookArticles(): Promise<SanityArticle[]> {
  const query = `*[_type == "bookArticle"] | order(order asc) {
    _id, title, category, readTime, description, link,
    "imageUrl": image.asset->url, order
  }`;
  const clientWithNoCache = sanityClient.withConfig({ requestTagPrefix: Date.now().toString() });
...
export async function fetchBrainResources(): Promise<SanityArticle[]> {
  const query = `*[_type == "brainResource"] | order(order asc) {
    _id, title, category, description, link,
    "imageUrl": image.asset->url, "buttonText": buttonText, order
  }`;
  const clientWithNoCache = sanityClient.withConfig({ requestTagPrefix: Date.now().toString() });
  return clientWithNoCache.fetch(query);
}
...
export async function fetchLivingWellArticles(): Promise<SanityArticle[]> {
  const query = `*[_type == "livingWellArticle"] | order(order asc) {
    _id, title, category, description, body, link,
    "imageUrl": headerImage.asset->url, order
  }`;
  const clientWithNoCache = sanityClient.withConfig({ requestTagPrefix: Date.now().toString() });
  return clientWithNoCache.fetch(query);
}
...
export async function fetchGardeningProducts(): Promise<SanityProduct[]> {
  const query = `*[_type == "gardeningProduct"] | order(order asc) {
    _id, title, category, description, link, buttonText, review,
    "imageUrl": image.asset->url, order
  }`;
  const clientWithNoCache = sanityClient.withConfig({ requestTagPrefix: Date.now().toString() });
  return clientWithNoCache.fetch(query);
