          ...
          <div className="grid md:grid-cols-2 gap-8">
            {resources.map((resource) => (
              <div key={resource._id} className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300">
                <div className="w-16 h-16 rounded-2xl bg-violet-50 flex items-center justify-center text-violet-500 mb-6">
                  {resource.category?.toLowerCase().includes('game') ? <Puzzle size={32} /> : 
                   resource.category?.toLowerCase().includes('learn') ? <Lightbulb size={32} /> : 
                   <Brain size={32} />}
                </div>
                ...
                <a 
                  ...
                  className="inline-flex items-center justify-center w-full bg-slate-900 text-white px-6 py-4 rounded-xl font-bold hover:bg-violet-600 transition-colors group"
                >
