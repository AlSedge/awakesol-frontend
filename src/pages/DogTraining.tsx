...
export default function DogTraining() {
  ...
  useEffect(() => {
    fetchDogResources()
      ...
      .catch(err => console.error("Error fetching dog resources:", err))
    ...
    fetchDogArticles()
      ...
      .catch(err => console.error("Error fetching dog articles:", err));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <main className="max-w-5xl mx-auto px-4 py-12 md:px-8">
        <div className="mb-12">
          <h2 className="text-4xl font-extrabold text-slate-900 mb-4 tracking-tight">Build a Stronger Bond</h2>
        ...
        {/* Dog Training Guides (articles) */}
        {articles.length > 0 && (
          <div className="mb-16">
            <h2 className="text-3xl font-extrabold text-slate-900 mb-8 tracking-tight">Dog Training Guides</h2>
            <div className="grid md:grid-cols-2 gap-8">
              {articles.map((article) => (
                <Link
                  to={`/learning/dogs/${article._id}`}
                  key={article._id}
                ...
                >
                  {article.imageUrl ? (
                    ...
                    <div className="h-48 bg-amber-50 flex items-center justify-center">
                      <Dog size={48} className="text-amber-200" />
                    </div>
                  )}
                  <div className="p-8 flex-grow flex flex-col">
                    <div className="text-sm font-bold text-amber-500 mb-2 uppercase tracking-wider">{article.category}</div>
                    <h3 className="text-2xl font-bold text-slate-900 mb-3 group-hover:text-amber-600 transition-colors">{article.title}</h3>
                    <p className="text-slate-600 font-medium leading-relaxed flex-grow">{article.description}</p>
                    <div className="mt-6 font-bold text-amber-600 flex items-center">
        ...
        {loading ? (
            ...
            {resources.map((resource) => (
              <div key={resource._id} className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col">
                ...
                {resource.imageUrl ? (
                  ...
                  <div className="h-56 w-full bg-amber-50 flex items-center justify-center">
                    {resource.category?.toLowerCase().includes('health') ? <Heart size={48} className="text-amber-200" /> : 
                     resource.category?.toLowerCase().includes('treat') ? <Bone size={48} className="text-amber-200" /> : 
                     <Dog size={48} className="text-amber-200" />}
                  </div>
                )}
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <div className="text-sm font-bold text-amber-500 mb-2 uppercase tracking-wider">{resource.category}</div>
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 border-dashed">
            <Dog size={48} className="mx-auto text-slate-300 mb-4" />
            <h3 className="text-2xl font-bold text-slate-900 mb-2">Check back soon!</h3>
            <p className="text-slate-500 max-w-md mx-auto">We are currently evaluating the best training programs and resources. Our top recommendations will appear here shortly.</p>
          </div>
        )}
      </main>
    </div>
