            ...
            {resources.map((resource) => (
              <div key={resource._id} className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300">
                <div className="w-16 h-16 rounded-2xl bg-amber-50 flex items-center justify-center text-amber-500 mb-6">
                  {resource.category?.toLowerCase().includes('health') ? <Heart size={32} /> : 
                   resource.category?.toLowerCase().includes('treat') ? <Bone size={32} /> : 
                   <Dog size={32} />}
                </div>
                ...
                <a 
                  ...
                  className="inline-flex items-center justify-center w-full bg-slate-900 text-white px-6 py-4 rounded-xl font-bold hover:bg-amber-500 transition-colors group"
                >
