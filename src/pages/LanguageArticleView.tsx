...
export default function LanguageArticleView() {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      const query = `*[_type == "languageArticle" && _id == $id][0] {
      ...
      sanityClient.fetch<SanityArticle>(query, { id })
        ...
        .catch(err => console.error("Error fetching language article:", err))
  ...
  if (!article) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        ...
        <Link to="/learning/languages" className="text-sky-600 hover:underline">Return to Language Guides</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      <header className="bg-white border-b border-slate-200 py-6 px-4 md:px-8 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <Link to="/learning/languages" className="flex items-center gap-2 text-slate-600 hover:text-sky-600 font-bold transition-colors">
            <ArrowLeft size={20} />
      ...
      <main className="max-w-3xl mx-auto px-4 pt-12 md:px-8">
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            <span className="bg-sky-100 text-sky-700 px-3 py-1 rounded-full text-sm font-bold uppercase tracking-wider">
              {article.category}
            </span>
          </div>
        ...
        <article className="prose prose-lg prose-slate max-w-none prose-headings:font-bold prose-h2:mt-12 prose-h2:mb-4 prose-a:text-sky-700 prose-a:font-bold hover:prose-a:text-sky-500 prose-img:rounded-2xl">
          {article.body ? (
            <PortableText
              value={article.body}
              components={{
